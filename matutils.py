...
def scaleMatrix(scale):
    ''' Create a scalar vector based on a single value '''
...
def translationMatrix(t):
    ''' Create a matrix for translation based on t '''
    n = len(t)
    T = np.identity(n+1,dtype='f')
    T[:n,-1] = t
    return T

#A series of functions that generate matrices for rotation around a given axis
...
#Functions for managing homogenous co-ordinates
def homog(v):
